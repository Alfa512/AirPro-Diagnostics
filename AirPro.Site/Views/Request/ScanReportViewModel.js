var ScanReportViewModel = function (options, data) {
    ...
    var mappings = {
        ...
        'validationRules': {
            create: function(vr) {
                return new ValidationRuleViewModel(vr.data);
            }
        }
    };
    ...
    self.userCompletedInd = ko.observable(false);
    self.completeOnLoad = ko.observable(self.completeReport());

    self.completeReportToggle = function () {
        $('.tooltip').hide();

        if (!self.completeReport()) {
            $('#report-validation').modal('show');
            return;
        }

        self.completeReport(!self.completeReport());
        self.userCompletedInd(!self.completeOnLoad() && self.completeReport());
    ...
    self.completeValidationToggle = function() {
        self.completeReport(!self.completeReport());
        self.userCompletedInd(!self.completeOnLoad() && self.completeReport());
    };
    ...
    self.allowSave = ko.pureComputed(function () {
        return !self.loadingData()
            && (!self.completeReport() || self.validationPassInd())
            && (!self.cancelReport() || self.cancelNotes() && self.cancelNotes().length > 0 && self.cancelReasonTypeId())
            && self.requestTypeId();
    });

    self.validationPassInd = ko.pureComputed(function () {
        return !self.validationRules() || self.validationRules().filter(function(rule) {
            return !rule.validationRuleResultInd() && !rule.resultAcknowledgedInd();
        }).length === 0;
    });
    ...
    function toJsonWithNullFields(objectWithObservables) {
        var obj = ko.mapping.toJS(objectWithObservables, propertyIgnore);
        for (var prop in obj) {
    ...
    self.hasChanges = ko.pureComputed(function () {
        var state = toJsonWithNullFields(self);

        return self.allowEdit() && (self.origState() !== state)
            && (self.anyToolSelected() || !self.oemCertifiedInd())
            && (self.versionEnteredForSelectedTools() || !self.anyToolSelected());
    });
    ...
    self.versionEnteredForSelectedTools = ko.pureComputed(function () {
        ...
        }).length === self.vehicleMakeTools().filter(function (f) {
            return f.checkedInd();
        }).length || self.cancelReport() || !self.allowEdit() || self.vehicleMakeTools().length == 0;
    });
};
...
var ValidationRuleViewModel = function(data) {
    var self = this;
    ko.mapping.fromJS(data, {}, self);

    self.resultCss = ko.pureComputed(function() {
        return self.validationRuleResultInd()
            ? 'text-severity-success'
            : self.resultAcknowledgedInd()
            ? 'text-severity-warning'
            : 'text-severity-danger';
    });

    self.resultText = ko.pureComputed(function() {
        return self.validationRuleResultInd() ? 'PASS' : 'FAIL';
    });
};
